  ...
  >
    ...
    >
      ...
      <div class="flex-c gap-2.5">
        ...
        <!-- 通知按钮（仅点击弹出/关闭抽屉） -->
        <div v-if="shouldShowNotification" class="notice-trigger-zone flex shrink-0">
          <div class="relative inline-block" @click.stop>
            <ArtIconButton
              icon="ri:notification-2-line"
              class="notice-button relative"
              @click.stop="handleNoticeClick"
            >
              <div
                v-if="hasUnreadNotice"
        ...
        <!-- 聊天按钮 -->
        <div v-if="shouldShowChat" class="relative inline-block">
          <ArtIconButton icon="ri:message-3-line" class="chat-button relative" @click="openChat">
            <div
              v-if="hasUnreadChat"
              class="absolute top-2 right-2 size-1.5 !bg-success rounded-full"
            ></div>
          </ArtIconButton>
        </div>
...
<script setup lang="ts">
  ...
  const { noticeList, msgList } = storeToRefs(notificationStore)
  const imChatStore = useImChatStore()
  const { hasUnreadMessages: hasUnreadChat } = storeToRefs(imChatStore)
  const hasUnreadNotice = computed(
    () => noticeList.value.some((n) => !n.read) || msgList.value.some((m) => !m.read)
  )
