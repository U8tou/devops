<!-- 项目标签字典 CRUD（与流程列表标签管理交互一致） -->
<template>
  <ElDialog
    v-model="visible"
    :title="$t('dev.project.tagManageTitle')"
...
<script setup lang="ts">
  ...
  import {
    fetchDevProjectTagAdd,
    fetchDevProjectTagDel,
    fetchDevProjectTagEdit,
    fetchDevProjectTagList
  } from '@/api/dev-project'
  import { useUserStore } from '@/store/modules/user'

  defineOptions({ name: 'ProjectTagManageDialog' })
  ...
  const canAddTag = computed(
    () =>
      buttons.value.includes('dev:project:edit') || buttons.value.includes('dev:project:tag:add')
  )
  const canEditTag = computed(
    () =>
      buttons.value.includes('dev:project:edit') || buttons.value.includes('dev:project:tag:edit')
  )
  const canDelTag = computed(
    () =>
      buttons.value.includes('dev:project:edit') || buttons.value.includes('dev:project:tag:del')
  )
  ...
  async function saveEdit(row: { id: string; name: string }) {
    ...
    try {
      await fetchDevProjectTagEdit({ id: row.id, name })
  ...
  async function addTag() {
    try {
      ...
      adding.value = true
      try {
        await fetchDevProjectTagAdd({ name })
        ElMessage.success(t('dev.process.tagSaveOk'))
        await load()
      } finally {
        adding.value = false
  ...
  async function remove(row: { id: string; name: string }) {
    ...
    try {
      await fetchDevProjectTagDel(row.id)
