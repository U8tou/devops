...
export class RouteTransformer {
  ...
  /**
   * 提取路径第一段（相对形式，用于作为根路由的子路由）
   */
  private extractRelativeSegment(path: string): string {
    const segments = path.split('/').filter(Boolean)
    return segments.length > 0 ? segments[0] : ''
  }

  /**
   * 转换路由配置（作为根 Layout 的子路由，避免每个模块独立使用 Layout 导致菜单重复渲染）
   */
  transformAsChildOfRoot(route: AppRouteRecord): ConvertedRoute {
    const { component, children, ...routeConfig } = route

    const converted: ConvertedRoute = {
      ...routeConfig,
      path: this.extractRelativeSegment(route.path || ''),
      component: undefined
    }

    if (route.meta?.isIframe) {
      this.handleIframeRoute(converted, route, 0)
    } else if (children?.length) {
      // 有子路由：使用空白布局占位，不使用完整 Layout
      converted.component = this.componentLoader.loadBlankView()
      converted.children = children.map((child) => this.transform(child, 1))
    } else {
      // 无子路由：直接使用页面组件
      // 重要：若配置了 Layout 组件，不能加载（已是 Root 子路由），否则会导致布局重复渲染
