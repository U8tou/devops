...
export class RouteRegistry {
  ...
  /**
   * 注册动态路由
   * 将菜单路由作为静态 Root 的子路由添加，确保单一 Layout、避免重复渲染
   */
  register(menuList: AppRouteRecord[]): void {
    ...
    // 将菜单路由作为 Root 的子路由添加（Root 已在 staticRoutes 中定义）
    for (const route of menuList) {
      if (route.name && !this.router.hasRoute(route.name)) {
        const childConfig = this.transformer.transformAsChildOfRoot(route)
        const removeRouteFn = this.router.addRoute(ROOT_ROUTE_NAME, childConfig as RouteRecordRaw)
